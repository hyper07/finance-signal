...
AI market signal products targeting retail investors increasingly offer multi-day directional forecasts. This study examines a seven-session historical-analog forecaster that has issued live, immutable daily signals for the Bitcoin ETF pair (BITO/BITI) on signal.dotori.ai since 2026-01-02. Using the unchanged production code, all forecasts that the model could have generated between 2022 and 2026 were reconstructed with an offline Alpaca price database (1,125 origins for BITO; 1,973 for BTC/USD spot), and the seven-horizon predictive distributions were evaluated against realized outcomes. Unconditionally, directional accuracy is 46–51% at every horizon on both instruments and never significantly exceeds 50%, while nominal-80% prediction intervals cover 78–82% of outcomes — the model is *interval-calibrated but directionally uninformative in calm conditions*. The analysis then considers exogenous information shocks defined as (i) spikes in headline news items (crypto-tagged newswire items at least twice the trailing median, not referencing returns) and (ii) standardized return shocks with $|z|\ge 2.5$. On headline spike days, the next-session directional accuracy of BTC forecasts drops to 39.0% (95% CI 29.8–49.0, $p=0.04$ compared to 50%), standardized forecast surprise increases by approximately one third (Mann–Whitney $p=0.02$), and interval coverage decreases by 10 percentage points; forecasts issued the day before a shock exhibit 0% coverage and a mean absolute surprise of 3.6 band-$\sigma$. After a shock, prices continue in the shock direction for a week (BTC: +2.98% sign-adjusted over seven sessions, $t=3.06$, $p=0.003$, 98 events), yet the model’s seven-vote consensus aligns with the shock direction in only 6–10% of events and requires a median of 5 to 6 sessions to reach agreement. A walk-forward, cost-adjusted rule indicates that continuation can be exploited only in Bitcoin and only if acted upon within one session (+2.3% net per event at 5 bp, 64% of events), while attempts to follow index or single-stock movements result in losses after accounting for costs. The 2026-08-19 White House crypto summit, during which Bitcoin rose by 21% over three sessions (the largest short liquidation in its history), is analyzed as a case study: on 2026-08-18, the model recommended a $-100\%$ (fully inverse) tactical position, and its seven-session P90 path peaked at +3.8%. The analysis demonstrates that the 60–65% accuracy figures recalled from the product’s 50-origin evaluator are consistent with sampling noise (standard deviation approximately 0.06–0.07 per reading; at least one of the seven horizons reached 60% or above on 14% of days in 2026). Formal sequential tests agree: no horizon passes the Pesaran–Timmermann directional test, the Brier score is significantly *worse* than the constant $\tfrac12$ forecast at nearly every horizon (HAC $t$ = 1.7–4.9), and the cumulative log Bayes factor against a fair coin is negative throughout ($-9$ to $-78$ nats, below zero 89–100% of the time). Extending the protocol to the deployed S&P 500 pair (SPXL/SPXS), SPY and 50 single stocks (2,584–4,097 origins each; 204,129 scored next-session stock forecasts) shows that the *same* engine fails *differently* by asset type: index shocks are two-thirds downward, revert ($-0.5$ to $-1.4\%$ over seven sessions) and cut the forecaster's accuracy from 0.60–0.62 to 0.34–0.36 inside shock windows, whereas single-stock shocks are idiosyncratic own-news jumps (own-ticker headline spikes triple the shock probability) on which the forecaster is neither better nor worse than in calm periods; at seven sessions the naive "always up" rule beats the forecaster on all eight instruments. Simulations of acting on the published schedule daily, weekly, or monthly reveal no dominant strategy—the rankings vary across instruments and time periods—and the committed weekly schedule differs from daily re-forecasting by at least 25 exposure points on 66–75% of sessions. A fully exogenous test on calendar events confirms the mechanism: on earnings days (known years in advance) the stocks' nominal-80% bands cover 25–27% of outcomes and forecast surprise is 3–4× larger ($p<10^{-5}$), on FOMC days SPY accuracy falls to 0.40 ($p=0.016$), and crypto is unaffected by FOMC days — the forecaster has no event calendar of any type; pooled over 50 stocks the Pesaran–Timmermann statistic is 3.08, a real but economically negligible 0.3-point directional dependence, while the Brier score is worse than a coin for 98% of stocks. A constructive test completes the analysis: a causal, event-conditional gate estimated in a walk-forward manner improves the interval score on earnings sessions by 16% across 50 stocks (coverage rises from 0.49 to 0.68, confidence interval excludes zero) and after crypto shocks by 7–17%, while the same rule negatively affects stocks and the index on calendar days—the choice of gate depends on event type. Under Benjamini–Hochberg control over all 228 reported tests, no directional-skill test survives, while distributional failures persist. The findings support the use of type-specific model families rather than a single shared indicator-voter set. The central thesis is stated as a bound: for a price-conditioned forecaster, directional accuracy is bounded above by the frequency and sign-unpredictability of information arrivals and below by the skill observed during calm periods; in this study, 3–5% of sessions account for 28–37% of return variance, calm-period accuracy is at chance level, and the returns that the product appears to capture derive from exposure, not prediction. The results indicate that human reaction to news constitutes an unmodeled state process—a jump followed by a behavioral response kernel—and a research program in mathematics and neuroscience is proposed to measure this kernel so it can be incorporated into forecasting models rather than remaining as unexplained error.
...
The production engine (in `model/strategy.py::run_simulation`) calculates about 40 binary technical indicator signals each day and, on each day, chooses the seven indicators which have the best hit rate against the direction three days ahead over a 10-day look-back period that ends three sessions before the decision date (thus ensuring that every label used has already been realised). A weighted vote results in a signal $S_t\in\{-1,0,+1\}$ (Short/Hold/Long). For BITO the Short position involves the inverse ETF BITI. Signals that have been published live since 2026-01-01 are stored in a cache and are never recomputed.
...
The empirical distribution of the 60 rescaled outcomes gives $\hat q_{10},\hat q_{50},\hat q_{90}$, $\hat p_h=\hat P(R>0)$, a per-session probability of an up move, and a Buy/Sell/Hold vote per horizon (Buy if $\hat q^{sess}_{50}>0$ and $\hat p^{sess}\ge0.55$; Sell symmetrically). A consensus of $\ge 6$ concordant votes is "confirmed" (full move to the target exposure), exactly 5 is a "warning" (half move). A separate schedule converts a score $0.45(2\hat p_h-1)+0.35\,\hat q_{50}/\text{risk}+0.20\,\pi_t$ into a tactical exposure path bounded in $[-100\%,100\%]$ for inverse-ETF datasets.

### 2.4 Offline reproduction and its fidelity

We rebuilt the production `detailed` frame from the Alpaca archive with the deployed `run_simulation`, freezing the 168 published 2026 signals exactly as the live service does. In order to assess how faithful the *unfrozen* engine is, we also ran it without any freezing and compared the results to the published records for the 165 overlapping days: the signal agreement was 76.4% (monthly breakdown: 70/95/36/71/85/91/91/75%); the adjusted-close discrepancy compared to the median of the published series was 0.004%, with a maximum of 0.20%. The disagreements occur due to (a) the different adjustment practices between Yahoo and Alpaca and (b) the sector-rotation attention feature, which involves eleven sector ETFs that we do not archive and which is set to its neutral value (0) when offline. Since the study makes use of the *frozen* frame, all of the 2026 results exactly match the signals that the public saw; the pre-2026 signals are the engine's own walk-forward outputs, as in production (and these regenerate the pre-live rows). The rotation feature is neutral in all of our analog distance measurements; this represents a known deviation from production and is the primary threat to an exact replication of the production forecasts. For the other deployed datasets, the unfrozen agreement is 84.2% for the S&P pair (over 165 days; median close gap 0.001%) and 93 to 100% for the four stocks (AAPL and NVDA at 100%, MSFT at 94%, TSLA at 93%; regarding the 25 live rows since 2026-07-30: 100/100/100/76%). The stock agreement is high for an uncomfortable reason: the cash-mode 200-day trend filter forces "Long" on 90–93% of all sessions and on every one of the 168 published 2026 rows for AAPL and NVDA, so there is little for the indicator voters to disagree about.
...
- **Headline spike.** $N_T$ = number of Benzinga items on day $T$ tagged BTCUSD/BITO/IBIT/BITI/MSTR/COIN or matching *bitcoin|crypto|btc|stablecoin|digital asset*; weekend items are attributed to the next session. The intensity ratio is defined as $\nu_T=(N_T+1)/(\tilde N^{(30)}_{T-1}+1)$, where $\tilde N^{(30)}$ is the median over the previous 30 days; a spike occurs when $\nu_T\ge 2$ is at least 2. Coverage started on 2024-02-01.
...
We use Wilson score in$t$ervals and exact binomial tests concerning hit rates; for the differences between the overlapping-horizon groups we apply a moving-block bootstrap (with a block length of 7 and 4,000 draws); Mann–Whitney U tests are used for the surprise distributions; Spearman rank correlation is employed to examine monotone dependence on news intensity; one-sample t tests are conducted for the continuation returns. Instead of reporting only the best horizon we report all the horizons and deal with the issue of multiplicity in Section 7.

### 3.6 Pre-specified rules and multiplicity

All event definitions (Section 3.3), scoring rules (3.2), horizons, the shock threshold, the news-intensity threshold and the policy cadences (6.4) were fixed before any forecast was scored, and no rule was changed after results were seen; sensitivity to the shock threshold is reported (Section 5.1). Because the study re$p$orts eight instruments × seven horizons × several conditioning splits, we group every reported p-value into pre-specified families (direction vs ½; Pesaran–Timmermann skill; Brier vs the coin; shock vs calm; pre-event direction; post-shock continuation; headline-spike direction and surprise; scheduled-event direction and surprise) and apply Benjamini–Hochberg false-discovery control within each family and across all families jointly (Section 4.4). For horizons h>1 the Pesaran–Timm4. For horizons $h>1$ the Pesaran–Timmermann statistic entering the family is the mean over the $h$ non-overlapping subsequences, not the overlap-inflated full-sample value.
...
The origins are $n=1{,}119$ to $1{,}125$ (BITO, 4 March 2022 to 27 August 2026) and $1{,}967$ to $1{,}973$ (BTC, 5 April 2021 to 29 August 2026). Both horizons are considerably *below* 0.5 (BITO with $h=3$ and $p=0.007$; BTC with $h=7$ and $p=0.004$), while none of them is significantly above this level. The Brier scores, ranging from 0.25 to 0.27, are at or worse than those of the fixed forecast $\hat p=0.5$ (Brier 0.25). When looking only at the year 2026 (the live period), the values are BITO 0.45 to 0.52 and BTC 0.41 to 0.50 (for BTC with $h=7$: 0.413, $p=0.009$). The per-session votes (considering only Buy and Sell) reach 49 to 52%.
...
The three standard sequential criteria are used to define "effectiveness in series", each of them being calculated over the entire origin sequence (see Table 2). (i) The **Pesaran–Timmermann (1992) statistic** is used to check if the predicted and actual directions are independent, allowing for unequal base rates; it has an asymptotic distribution of $N(0,1)$, and any values exceeding 1.645 indicate a degree of skill at the 5% one-sided level. For $h>1$ consecutive outcomes overlap, so we also compute it on the $h$ non-overlapping subsequences (every $h$-th origin) and report their mean and range. (ii) A **Diebold–Mariano-type test** of the Brier loss against the constant forecast $\hat p\equiv\tfrac12$ (loss 0.25): $d_t=(\hat p_t-y_t)^2-0.25$, with Newey–West long-run variance (Bartlett kernel, lag $h$); a positive mean means the model is worse than the coin. (iii) The **cumulative log Bayes factor** $\Lambda_n=\sum_{t\le n}\big[y_t\log\hat p_t+(1-y_t)\log(1-\hat p_t)+\log 2\big]$ (with the probabilities clipped to $[0.02,0.98]$) is used, and under calibration the expected value per step is equal to the Kullback–Leibler divergence $\mathrm{KL}(\hat p_t\,\|\,\tfrac12)\ge0$ — therefore an effective probabilistic forecaster generates a submartingale that tends to increase, and the even-odds Kelly bet of fraction $2\hat p_t-1$ has a positive expected log growth. We give $\Lambda_n$, the proportion of the sequence that lies below zero, and the realized Kelly growth per session.
...
The analysis in Table 13 a$p$plies Benjamini–Hochberg control to all of the p-values given in the paper (covering 228 tests across ten pre-determined families; Section 3.6). The pattern is the paper's argument in one table. **No Pesaran–Timmermann skill test survives at any horizon on any instrument (0 of 56, within-family or pooled).** The *distributional* findings survive: the Brier score is worse than the constant forecast on 22 of 56 instrument–horizon pairs; the shock-window accuracy deficit survives on 5 of 8 instruments (BITO, SPXL, SPY, AAPL, MSFT); forecast surprise is larger on scheduled-event days in 10 of 16 instrument–calendar pairs (all four earnings sets, payrolls on SPY and SPXL, and BTC payrolls) and on headline-spike days for both crypto instruments. Of the 34 results that are significantly above ½ and survive, all pertain to index or single-stock horizons where the always-up base rate is higher than the model’s accuracy — that is, base-rate inflation rather than skill — and the two that survive *below* ½ are related to crypto horizons. The per-event *directional* results are the paper's weakest links once multiplicity is charged: the 37% pre-shock accuracy (BTC $q$=0.09 within family, 0.035 pooled; BITO $q$=0.26), the 39% headline-spike accuracy (BTC $q$=0.08) and the FOMC-day accuracy drop on SPY ($q$=0.60) are suggestive at $q\le0.10$ or weaker and should be read as such; the BTC seven-session continuation survives ($q$=0.023). The conclusions on which the paper is based — the absence of skill, confident miscalibration, and the degradation of the predictive distribution on days with exogenous events — are the ones that survive.
...
The collapse in the coverage is to some extent due to mechanical factors (as stated in Section 1). However, the **directional** outcome is not: with respect to BITO, forecasts which have a target span including a shock are incorrect 64% of the time, a 14-point shortfall that remains significant at every threshold from $|z|\ge2.0$ to $3.5$ (the proportion for shock-window hits being 0.36–0.40 compared to 0.49–0.51 for calm periods). For BTC the deficit is smaller (0.46 compared to 0.49) and is not significant.
...
Headline spikes are only weakly associated with large returns (2.7–2.8× the base rate of $|z|\ge2.5$ days, but 88–92% of spike days are *not* z-shocks), so this test is largely independent of Section 5.1. Yet on spike days the forecast surprise is considerably greater for both instruments, coverage decreases by about 10 points, and for BTC the directional accuracy drops to 39%. Across the whole news-covered sample, Spearman correlations of $\nu_T$ with coverage are −0.11 (BITO, $p=0.004$) and −0.18 (BTC, $p<10^{-4}$), and with $|s|$ +0.15 and +0.23 ($p\le10^{-4}$); the correlation with the *direction* hit is ≈0 for both (Fig. 4, right). **News degrades the model's distribution, not just its sign: the more headlines, the further the outcome lands from the forecast median in the forecast's own units.**

On the day before the spike (for all seven horizons) the forecast *paths* reached 0.43 (BITO) and 0.45 (BTC) with coverage figures of 0.58 and 0.66, whereas on other days the figures were 0.50/0.81 and 0.49/0.79.
...
The 3-day signal matches the upcoming shock as frequently as a coin toss, whereas the seven-session consensus almost never does (Fig. 7).
...
The service in use employs one indicator-voter engine and one analog forecaster for crypto, the S&P 500 pair and several hundred individual stocks; the only differences being the cash-mode trend/multi-horizon filters and the inverse leg. When the same protocol is applied to the deployed S&P dataset (SPXL/SPXS, with the 2026 signals frozen), against SPY spot and the four key stocks (AAPL, MSFT, TSLA, NVDA; with the live signals frozen from 2026-07-30), it is found that the common engine is addressing a different prediction problem in each of the asset types. (Fig. 9, 10)
...
Headline counts are a proxy; return shocks are partly circular. Public calendars are neither: FOMC decision days (2016 → 2026-07, 85 sessions), Employment Situation releases (first Friday, 123 sessions) and each stock’s quarterly earnings reaction session (63–65 per stock, from 2010) are known in advance and never reference prices or headlines. We compare the $h=1$ forecast issued the day before each event with all other forecasts (Table 12). Table 12.
...
Three observations. First, **every catalogued increase is an information event** — an earnings print, a policy or market-wide day, an analyst target, a product or management announcement, a large order, a legal settlement — and none is a pattern in the prior price path; this is the single-stock version of Proposition 1. Second, in every category the forecasting model provided forecast bands of about ±2 to ±4% for moves ranging from +12% to +30%, with the collective view agreeing on 15 to 24% of the events (this is better than the figure for crypto, which is 6 to 10%, since the cash-mode engine is long most of the time, not because it had anticipated the news). Third, the categories behave differently *after* the event, which matters for the type-specific design of Section 8.3: earnings increases keep drifting (+2.3% over seven sessions, $t$=2.7), reproducing post-earnings-announcement drift; idiosyncratic-news increases drift weakly (+1.1%, $t$=1.6); market-wide increases give back part of the move the next day (−0.65%) and then flatten. The stock model should therefore include an earnings calendar with a drift component, regard market-wide days as equivalent to index exposure, and treat other company news as a jump with no follow-through — that is, three distinct regimes rather than a single voter set.

### 5.7 The stock cross-section: 50 S&P 500 names

In order to determine if the four stocks are representative, the procedure was carried out on 46 additional companies which are constituents of the S&P 500 and cover all sectors having published `s00` datasets (the live signals were frozen on 2026-07-30; the histories extend back to 2010, and 204,129 next-session forecasts were scored; Berkshire Hathaway was excluded since it lacked tagged headlines). Table 17 summarises the cross-section; the per-stock table is `output/universe_cross_section.csv`.
...
The cross-section not only sharpens the four-stock picture in one respect but also supports it in all the other respects. When 204,129 forecasts are pooled together, the Pesaran–Timmermann stat*is*tic comes to 3.08: there is a statistically identifiable directional dependence at one session in individual stocks — amounting to 0.32 percentage points above the level of independence, and with 8% of the stocks exceeding the 5% critical value as against 4% falling below it. The economic significance of this is negligible and is overwhelmed by miscalibration: for 98% of the stocks the Brier score is worse than a constant ½ (and this is significant for 88% of them), the always-up rule outperforms the model at seven sessions for 98% of the stocks, and on earnings sessions the nominal-80% band includes 28% of the outcomes for the median stock and covers less than the amount it covers during calm sessions for each of the 48 stocks with earnings dates. For the median stock the behaviour after a shock is only very slightly mean-reverting (by -0.18% over seven sessions; only 32% of the stocks continue), the contrary of what is seen in crypto. (Fig. 12.)
...
With regard to BTC (98 events), the drift is monotonic and significant from the second session on: a trader who simply follows the move on the news day ends up with about 3% over the following week, in 62% of cases the outcome being positive. When broken down by direction, up-shocks continue to yield +3.8% after seven sessions and down-shocks result in −1.9% (for BTC); for BITO, up-shocks continue to give +3.4% while down-shocks show a slight reversal (+0.4%, with $n=22$). This is exactly the kind of post-announcement drift or under-reaction that is described in the behavioural finance literature (Barberis et al., 1998; Chan, 2003; Hong & Stein, 1999), the pattern being observed in a 24/7 asset with the exception of the small sample size.
...
The answer is narrow. **For Bitcoin, yes — within one session.** Entering at the shock close or the next close earns +2.51% and +2.33% net per event at 5 bp (61%/64% of events positive, $t$ = 2.32/2.5) and +1.93% at 25 bp; the trailing filter chose continuation at every decision, so the rule is out of sample only in the sense that 21 prior events were required, and it held in 2025–26 (+2.36%, 70% hit, 30 events). By the second session the edge halves (+1.34%, $t$ = 1.64) and is no longer significant; by the third it is weaker still; by the fifth it is gone. BITO has too few events for the filter to fire. **For the index, no:** chasing loses (-0.56% per event at 5 bp, -0.96% at 25 bp, $t$ = -2.36) and the reversal the filter selects earns nothing net. **For single stocks, no as a rule:** across 1,390 earnings shocks in 50 names the walk-forward continuation is +0.25% per event at 5 bp and -0.15% at 25 bp — the +2.3% of Table 14 was the four mega-caps' upward jumps, not the universe — idiosyncratic moves have no drift, and market-wide moves reverse (+0.30% at 5 bp, $t$ = 2.65; -0.10% at 25 bp). The per-event dispersion is 7–10% against a 2–3% mean even in the Bitcoin case, so roughly four events in ten lose money at the best entry. The defensible product statement is therefore descriptive and type-specific: after a crypto shock, do not fade it, and any action taken should be taken within one session and held about a week; after index shocks and market-wide days, do not chase; after earnings and other company news, the historical drift is too small to survive realistic costs.

### 6.2 How long the model takes to agree

If the system is not aligned at time $T-1$, the 3-day signal matches the direction of the shock for a median of 2 (in the case of BITO) and 3 (in the case of BTC) sessions; out of the 98 BTC events, 24 never showed agreement within 15 sessions. For the seven-vote consensus, a median of 5 (for BITO) and 6 (for BTC) sessions is required (see Fig. 7). Given the drift in Table 7, **the model's agreement arrives after most of the exploitable continuation has passed**, and on the BTC instrument the 3-day signal's cash-mode trend filter forces "Long" for long stretches regardless, so "alignment" there is partly coincidental.
...
The payload consists of a seven-session exposure schedule, allowing the user to deal with it in various ways. We simulate, point-in-time and with 5 bp per unit of turnover, (a) **act daily** — set exposure to each day's fresh $h=1$ target; (b) **act weekly** — commit to the seven-row schedule and follow it without re-forecasting, re-planning every seven sessions; (c) **act once, hold** — take the $h=1$ target and change nothing for seven sessions; against (d) buy-and-hold and (e) the deployed 3-day signal. In the case of negative exposure, the actual inverse-ETF return (BITI, SPXS) is earned. (Table 11, Fig. 11)
...
If the problem is that there is no event state variable, then the addition of such a variable—even in a very rough manner—should improve the predictive distribution precisely at the places where the paper states that it fails and at no other places. We test this out of sample with **causal gates** built only from information available at the origin and thresholds fixed in Section 3: *calendar* (the target session is an FOMC/payroll day for the index or an earnings reaction session for a stock), *headline* (the previous session's own-news ratio $\nu_{t-1}\ge2$), *shock* (the origin session itself was a $\lvert z\rvert\ge2.5$ move). On gated forecasts the model's quantiles are replaced by an **event-conditional distribution estimated walk-forward from the past only**: the empirical quantiles of past same-type event sessions (earnings, FOMC/payroll; ≥8 events), or the sign-adjusted post-shock continuation distribution (≥10 shocks); otherwise a trailing 250-window climatology. The scores are the Winkler interval score (with α=0.2), coverage and Brier; the differences are obtained by block bootstrapping. (See Table 18 and Fig. 13.) Table 18.
...
which for Gaussian $\varepsilon$ and $|\mu_t|\ll\sigma_t$ is $\le 2\Phi(-c)$ ($=0.012$ at $c=2.5$). In particular $A(E_c)\le\max(q,1-q)+2\Phi(-c)$, and if news direction is unpredictable ($q=\tfrac12$) every price-conditioned predictor has news-day accuracy $\tfrac12\pm0.012$ — the predictor is irrelevant.
...
After large price movements the analog forecaster is contrarian since the firms nearby are situated in periods characterised by calmness in which large one-day moves tend to partly reverse; out of the 98 cases of BTC shocks and 48 cases of BITO shocks, 21 and 10 respectively were followed by a further shock within three sessions, almost always of the same sign (the clearest example being the period from Aug 19 to 21 2026). Proposition 2 thus predicts the observed lack of skill on shock days (see Table 3: PT = $-2.49$ for BTC; accuracy at $h=1$ is 0.37).
...
which equals $0.111$ at $c=2.5$ and $0.013$ at $c=3.5$. Sin$0.000$e the observed coverage of $|z|\ge2.5$ for days with $c$ is within the mechanical bound, it contains no information regarding the forecaster; it is the coverage statistics conditioned on headline intensity (Table 5) that are informative, and these do not fix the value of c.
...
**What is and is not mechanical.** Zero interval coverage on $|z|\ge2.5$ days at $h=1$ is essentially implied by the definitions (Section 1) and we do not count it as evidence. The evidence is: (i) directional accuracy *below* 50% on the largest moves and on headline-spike days; (ii) monotone degradation of coverage and surprise with a news measure that never sees returns; (iii) the drift after shocks; (iv) the latency of the model’s agreement. (i)–(iv) are jointly what one expects from a forecaster whose analog pool is dominated by calm, mean-reverting regimes and whose features cannot represent “information has arrived”.
...
where $\tau_k$ are news arrival times, $\xi_k$ the impact sizes, and $K(\cdot;\theta)$ a *response kernel* describing how the population of traders digests the news over the following sessions (Merton 1976 for the jump; the kernel is the behavioral addition). A method based on the nearest neighbour approach is able to estimate only the conditional distribution of $r$ given $x_{t-1}$. As $x_{t-1}$ contains no information regarding $\tau_k$ or $\xi_k$, the forecast at time $T-1$ is unable to detect the jump, and more seriously, since the kernel $K$ is not available in the state, the forecasts at times $T$ and $T+1$ observe a large positive value of $r_T$ and, relying on calm analogues, predict a partial reversal, which is precisely the opposite of the drift shown in Table 7. Shocks also tend to cluster — 10 out of 48 (in the case of BITO) and 21 out of 98 (in the case of BTC) shock days are followed by another shock within three sessions (August 19, 20 and 21 were consecutive z-shocks; 2026-02-05/06; 2024-08-05/08) — indicating self-excitation (Bacry et al., 2015; Hawkes, 1971) both in the arrival of news and in the human response.

**Type-specific failure modes.** Section 5.4 shows the shared engine fails in three different ways: on crypto it is contrarian into clustered, continuing news shocks (anti-skill); on the index its directional accuracy is base-rate inflation and it collapses on the predominantly downward, heavy-tailed shocks (−26 points) even though its contrarian bias is directionally right afterwards (fast re-alignment); on single stocks the trend filter degenerates to "always Long" and shocks are idiosyncratic earnings/product news on which the analog pool carries no information either way. It is impossible for a single set of indicators, a single selection window, a single vote threshold and a single analog feature vector to be properly specified at the same time for a 24/7 momentum-prone asset, a mean-reverting negatively skewed index and a cross-section of idiosyncratic jump processes.

**A portfolio corollary.** The same taxonomy explains why the index, which has no earnings of its own, is the instrument on which long-horizon investing has been stable: an index of five hundred idiosyncratic information streams cancels exactly the risk this paper shows to be unforecastable, leaving scheduled macro news, shocks that revert (Table 8) and the equity premium behind the always-up base rate of Table 9. The approach also includes the traditional 60/40 stock–bond strategy with rebalancing—that is, selling bonds to buy stocks following a decline and carrying out the reverse operation after a rise—which is a *concave*, contrarian rule within the taxonomy developed by Perold and Sharpe (1988): it yields profits when markets are oscillating and offsets its insurance costs when they are trending, with a rebalancing bonus equal to about half the variance eliminated (Bernstein & Wilkinson, 1997; Hallerbach, 2014). On this archive the strategy performs as the type results predict (Appendix E, Table 21, Fig. 14): on the SPY/TLT portfolio over the period 2016–2026, a rebalanced 60/40 allocation reduced volatility from 17.5% to 11% and the maximum drawdown from −34% to −27%, without increasing returns or the Sharpe ratio, since long-term Treasuries lost money and declined in line with the stocks in 2022 (the daily return correlation between SPY and TLT was −0.4 from 2016 to 2020 and became positive in every year from 2022; see Appendix E); on BTC/TLT the rebalancing bonus of an asset with 58% volatility generated additional returns, while buying at the bottom of the week following a crypto market shock resulted in a loss—the continuation of Section 6.1 once again. Constant-mix rebalancing is therefore the natural policy for the index type and the incorrect reaction to take during the first week after a crypto market shock; we present it here as an example, not as an asset allocation recommendation, since such a conclusion would require longer data samples, a standard bond proxy, and tests comparing Sharpe ratios.
...
**Limitations.** (1) Offline reproduction differs from production in the rotation feature and price source (76% unfrozen signal agreement); the 2026 signals are exact. (2) News intensity is measured by the number of Benzinga headlines, which is a crude proxy for the level of information, salience and sentiment. (3) There are two crypto instruments, two index instruments and 50 stocks; the crypto window spans from 4.5 to 5.5 years and includes 48 and 98 shock events; the 46 stocks in the universe were not individually checked for fidelity against the production. (4) The overlapping time horizons lead to dependence; although we use block bootstraps, the effective sample size is less than the number of rows. (5) With seven time horizons and multiple splits there is a potential for multiplicity; we report all the horizons and the reader is advised to regard results with $p\approx0.03$ as merely suggestive. (6) Survivorship is not a problem (since there is only one instrument), but there is regime dependence: the period from 2024 to 2026 includes an ETF-approval bull run, a 2026 drawdown and a policy-driven rally.

### 7.3 The predictability gap

The data presented in Table 16 test the ceiling-and-floor argument of Proposition 4. On every instrument, sessions with $|z|\ge2.5$ are only 3.4–5.0% of the sample yet carry **28.5–36.5% of the return variance** and 13–18% of absolute price movement; headline-spike days add a further 5–14% of sessions. It is on these sessions that, by Proposition 1, the directional accuracy of a price-conditioned model is brought close to the news-sign base rate (which is observed to be 0.37–0.56) and its interval coverage is mechanically close to zero. The accuracy during calm sessions — the only circumstance in which skill could possibly exist — is 0.49–0.53, making it no different from that of a coin and *below* the always-up base rate on the instruments that are drifting. The arithmetic shows that if the model had possessed 60% (70%) directional skill during calm sessions, its overall accuracy could still not have gone above approximately 0.60 (approximately 0.69). The 60–70% figures advertised by retail signal products are therefore not a plausible *average*; they are the *ceiling under substantial calm-period skill that this model — and, on the evidence of Section 1.1, most technical models — does not have*. The gap that this paper measures lies between the ceiling and the coin: about one third of all price variance occurs on days which the model classifies as ordinary and nothing that is based on past prices can account for that one third.
...
There are two limitations with the gap measure. Firstly, it makes use of the return-defined shock set, which means that the proportion of unpredictable events represents a lower bound—this bound can be increased by including headline spikes and calendar sessions (Sections 5.2 and 5.5); and secondly, if a model had a truly different information set (for example, news text, order flow, or options positioning), it would not be subject to Proposition 1, since the proposition only places a bound on *price-conditioned* predictors, whereas the deployed engine and the technical-rule literature deal with exactly that.

---

## 8. Future research: measuring the human response kernel (mathematics × neuroscience)

A key empirical fact presented in this paper is that the errors made by a pattern-based forecaster are not white; rather, they occur around and following news arrivals and exhibit a *sign* (the model fails to capture movements that continue). That structure is produced by people. Hence, in order to survive news events a model must include a model of how people react to news — a question that falls within the domain of neuroscience and has a mathematical interface.

**8.1 Mathematical program.**
1. *State augmentation.* Add to $x_t$ an observable news-intensity process $\nu_t$ and the last standardized surprise $s_{t-1}$; estimate the conditional law of $r$ given $(x_{t-1},\nu_{t-1},s_{t-1})$ and test whether the Table 7 drift becomes predictable out of sample (purged, embargoed walk-forward as in López de Prado 2018). 2. *Kernel identification.* Estimate $K(\cdot;\theta)$ non-parametrically from the cross-section of shocks (a deconvolution/Volterra problem); test parametric forms — exponential decay (single-speed reaction), gamma (delayed peak, consistent with gradual information diffusion, Hong & Stein 1999), and mixtures (fast algorithmic + slow discretionary populations). 3. *Self-excitation.* Fit marked Hawkes processes to shock arrivals with headline marks; compare branching ratios in calm and policy-heavy regimes. 4. *Regime gating.* Hidden-Markov or change-point gating (Hamilton, 1989) that switches from the analog forecaster to an abstention or a drift model when the posterior probability of the "news regime" exceeds a threshold; evaluate with CRPS, not accuracy.
...
When evaluated exhaustively and point-in-time, the deployed seven-session analog forecaster for Bitcoin instruments is directionally uninformative (46–51% at every horizon), well-calibrated in width during calm periods, and systematically incorrect in both sign and width when exogenous news arrives: 37–39% next-session accuracy on shock and headline-spike days, zero interval coverage the day before a shock, and a mean surprise of 3.6 band-$\sigma$. Prices continue in the shock direction for approximately a week, while the model’s consensus requires 5–6 sessions to align. The 2026-08-19 CLARITY-Act rally, during which the model recommended a fully inverse position, exemplifies this failure. The underlying issue is structural: human response to information is not a function of past prices, indicating the need for a combined mathematical and neuroscientific approach to measure and model this response. The failure is also type-specific: the same engine is contrarian during continuing shocks in crypto, base-rate-inflated and collapse-prone on the negatively skewed index, and ineffective for single stocks experiencing idiosyncratic own-news jumps; no single schedule (daily, weekly, once) dominates across instruments. The constructive test demonstrates the benefit of such families: an event-conditional gate estimated solely from historical data restores coverage on earnings sessions (0.49→0.68 across 50 stocks, interval score +15%) and after crypto shocks, while applying the incorrect gate for a type is detrimental. The gap is quantified: 3–5% of sessions account for 28–37% of return variance and are unpredictable in sign from prices, so even 60–70% calm-period skill would cap accuracy near 0.60–0.69, and observed calm-period skill is negligible. For users, the operational guidance is narrow: after a crypto shock, act within one session or not at all; after index and stock moves, do not chase. Addressing this gap requires modeling the human reaction to news and social media—the attention, arousal, and herding chain described in Section 8.4—rather than the subsequent price path.
