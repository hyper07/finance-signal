...
Retail-facing "AI market signal" products increasingly publish multi-day directional forecasts. We study a deployed one: a seven-session historical-analog forecaster that has been producing live, immutable daily signals for a Bitcoin ETF pair (BITO/BITI) since 2026-01-02 on signal.dotori.ai. Using the production code unchanged, we regenerate every point-in-time forecast the model could have issued between 2022 and 2026 on an offline Alpaca price archive (1,125 origins for BITO; 1,973 for BTC/USD spot) and score the seven-horizon predictive distributions against realized paths. Unconditionally, directional accuracy is 46–51% at every horizon on both instruments and never significantly exceeds 50%, while nominal-80% prediction intervals cover 78–82% of outcomes — the model is *interval-calibrated but directionally uninformative in calm conditions*. We then condition on exogenous information shocks defined two ways: (i) headline-count spikes (crypto-tagged newswire items at least twice their trailing median), which do not reference returns; (ii) standardized return shocks $|z|\ge 2.5$. On headline-spike days the next-session directional accuracy of BTC forecasts falls to 39.0% (95% CI 29.8–49.0, $p=0.04$ against 50%), standardized forecast surprise rises by roughly one third (Mann–Whitney $p=0.02$), and interval coverage falls by 10 points; forecasts issued the day before a shock show 0% coverage and mean absolute surprise of 3.6 band-$\sigma$. Post-shock, prices continue to drift in the shock direction for a week (BTC: +2.98% sign-adjusted at seven sessions, $t=3.06$, $p=0.003$, 98 events), yet the model's seven-vote consensus was aligned with the shock direction before only 6–10% of events and required a median of 5–6 sessions to agree afterwards; a walk-forward, cost-adjusted rule shows the continuation is exploitable only in Bitcoin and only when acted on within one session (+2.3% net per event at 5 bp, 64% of events), while chasing index or single-stock moves loses after costs. The 2026-08-19 White House crypto summit — Bitcoin +21% in three sessions, the largest short liquidation in its history — is analyzed as a case study: on 2026-08-18 the deployed model recommended a $-100\%$ (fully inverse) tactical position and its seven-session P90 path topped out at +3.8%. We show that the 60–65% accuracies remembered from the product's 50-origin evaluator are consistent with sampling noise (SD ≈ 0.06–0.07 per reading; at least one of seven horizons read ≥60% on 14% of days in 2026). Formal sequential tests agree: no horizon passes the Pesaran–Timmermann directional test, the Brier score is significantly *worse* than the constant $\tfrac12$ forecast at nearly every horizon (HAC $t$ = 1.7–4.9), and the cumulative log Bayes factor against a fair coin is negative throughout ($-9$ to $-78$ nats, below zero 89–100% of the time). Extending the protocol to the deployed S&P 500 pair (SPXL/SPXS), SPY and 50 single stocks (2,584–4,097 origins each; 204,129 scored next-session stock forecasts) shows that the *same* engine fails *differently* by asset type: index shocks are two-thirds downward, revert ($-0.5$ to $-1.4\%$ over seven sessions) and cut the forecaster's accuracy from 0.60–0.62 to 0.34–0.36 inside shock windows, whereas single-stock shocks are idiosyncratic own-news jumps (own-ticker headline spikes triple the shock probability) on which the forecaster is neither better nor worse than in calm periods; at seven sessions the naive "always up" rule beats the forecaster on all eight instruments. A simulation of acting on the published schedule daily, weekly or once finds no dominant policy — rankings flip across instruments and periods — while the committed weekly schedule differs from daily re-forecasting by at least 25 exposure points on 66–75% of sessions. A fully exogenous test on calendar events confirms the mechanism: on earnings days (known years in advance) the stocks' nominal-80% bands cover 25–27% of outcomes and forecast surprise is 3–4× larger ($p<10^{-5}$), on FOMC days SPY accuracy falls to 0.40 ($p=0.016$), and crypto is unaffected by FOMC days — the forecaster has no event calendar of any type; pooled over 50 stocks the Pesaran–Timmermann statistic is 3.08, a real but economically negligible 0.3-point directional dependence, while the Brier score is worse than a coin for 98% of stocks. A constructive test closes the loop: a causal, event-conditional gate estimated walk-forward improves the interval score on earnings sessions by 16% across 50 stocks (coverage 0.49→0.68, CI excludes zero) and after crypto shocks by 7–17%, while the same rule hurts stocks after shocks and the index on calendar days — which gate to apply is itself type-specific. Under Benjamini–Hochberg control over all 228 reported tests, no directional-skill test survives while the distributional failures do. We argue for type-specific model families rather than a shared indicator-voter set. The thesis, stated as a bound: for a price-conditioned forecaster, directional accuracy is bounded above by the frequency and sign-unpredictability of information arrivals and below by calm-period skill; here 3–5% of sessions carry 28–37% of return variance, calm-period accuracy is at chance, and the returns the product appears to capture come from exposure, not prediction. We argue that news-driven human reaction is an unmodeled state process — a jump followed by a behavioral response kernel — and outline a research program in mathematics and neuroscience to measure that kernel so it can be built into forecasting models rather than absorbed as unexplained error.
...
Price-pattern forecasting models — technical-indicator ensembles, nearest-neighbour ("analog") predictors, and small supervised classifiers — condition only on the past path of the price itself. Their implicit assumption is that the state variables that matter for the next several sessions are functions of that path. Exogenous information (a policy announcement, a regulatory decision, an executive statement) violates this assumption: the information arrives as a jump that no price feature anticipates, and the market's *response* to it is produced by human agents whose behavior (attention, herding, forced liquidation, delayed reaction) is not in the model's state space either.
...
1. **Exhaustive, point-in-time evaluation.** Every origin between the warm-up period and the end of the archive is scored, not a 50-origin sample. All features and analog pools are computed from data observed at or before the origin; the code path is the one that publishes live.
2. **Two independent event definitions.** Return-based shocks ($|z|\ge 2.5$) are the natural definition but partially tautological for interval coverage, since a $2.5\sigma$ move lies outside a $\pm 1.3\sigma$ band by construction. We therefore also use a headline-count definition that never looks at the return.
...
The Alpaca archive stores two prints per (symbol, day) — IEX and consolidated SIP; we keep the higher-volume (SIP) row. Adjusted prices are used for indicators, matching production (`auto_adjust=True` in the deployed fetcher).

### 2.2 The deployed signal engine (3-day classifier)

The production engine (`model/strategy.py::run_simulation`) computes ~40 binary technical indicator signals daily and, each day, selects the seven indicators with the best hit rate against the 3-day-ahead direction over a 10-day look-back that ends three sessions before the decision date (so every label used is already realized). A weighted vote yields a signal $S_t\in\{-1,0,+1\}$ (Short/Hold/Long). For BITO the Short leg holds the inverse ETF BITI. Signals published live since 2026-01-01 are frozen in a cache and never recomputed.
...
We rebuilt the production `detailed` frame from the Alpaca archive with the deployed `run_simulation`, freezing the 168 published 2026 signals exactly as the live service does. To measure how faithful the *unfrozen* engine is, we also ran it with nothing frozen and compared to the published record on the 165 overlapping days: signal agreement 76.4% (by month: 70/95/36/71/85/91/91/75%); adjusted-close discrepancy versus the published series median 0.004%, maximum 0.20%. Disagreements arise from (a) Yahoo versus Alpaca adjustment conventions and (b) the sector-rotation attention feature, which requires eleven sector ETFs we do not archive and is set to its neutral value (0) offline. Because the study uses the *frozen* frame, all 2026 results reflect exactly the signals the public saw; pre-2026 signals are the engine's own walk-forward output, as in production (which regenerates pre-live rows). The rotation feature is neutral in all our analog distances; this is a documented deviation from production and the main threat to exact replication of production forecasts. The same check for the other deployed datasets gives 84.2% unfrozen agreement for the S&P pair (165 days; median close gap 0.001%) and 93–100% for the four stocks (AAPL and NVDA 100%, MSFT 94%, TSLA 93%; on the 25 live rows since 2026-07-30: 100/100/100/76%). The stock agreement is high for an uncomfortable reason: the cash-mode 200-day trend filter forces "Long" on 90–93% of all sessions and on every one of the 168 published 2026 rows for AAPL and NVDA, so there is little for the indicator voters to disagree about.
...
- **Headline spike.** $N_T$ = number of Benzinga items on day $T$ tagged BTCUSD/BITO/IBIT/BITI/MSTR/COIN or matching *bitcoin|crypto|btc|stablecoin|digital asset*; weekend items are attributed to the next session. The intensity ratio is $\nu_T=(N_T+1)/(\tilde N^{(30)}_{T-1}+1)$ with $\tilde N^{(30)}$ the trailing 30-day median; a spike is $\nu_T\ge 2$. Coverage begins 2024-02-01.
...
Wilson score intervals and exact binomial tests for hit rates; moving-block bootstrap (block length 7, 4,000 draws) for differences between overlapping-horizon groups; Mann–Whitney U for surprise distributions; Spearman rank correlation for monotone dependence on news intensity; one-sample $t$ tests for continuation returns. We report every horizon rather than the best one and discuss multiplicity in Section 7.

### 3.6 Pre-specified rules and multiplicity

All event definitions (Section 3.3), scoring rules (3.2), horizons, the shock threshold, the news-intensity threshold and the policy cadences (6.4) were fixed before any forecast was scored, and no rule was changed after results were seen; sensitivity to the shock threshold is reported (Section 5.1). Because the study reports eight instruments × seven horizons × several conditioning splits, we group every reported $p$-value into pre-specified families (direction vs ½; Pesaran–Timmermann skill; Brier vs the coin; shock vs calm; pre-event direction; post-shock continuation; headline-spike direction and surprise; scheduled-event direction and surprise) and apply Benjamini–Hochberg false-discovery control within each family and across all families jointly (Section 4.4). For horizons $h>1$ the Pesaran–Timmermann statistic entering the family is the mean over the $h$ non-overlapping subsequences, not the overlap-inflated full-sample value.
...
$n=1{,}119$–$1{,}125$ origins (BITO, 2022-03-04 → 2026-08-27) and $1{,}967$–$1{,}973$ (BTC, 2021-04-05 → 2026-08-29). Two horizons are significantly *below* 0.5 (BITO $h=3$, $p=0.007$; BTC $h=7$, $p=0.004$); none is significantly above. Brier scores of 0.25–0.27 are at or worse than the constant forecast $\hat p=0.5$ (Brier 0.25). Restricting to 2026 (the live period) gives BITO 0.45–0.52 and BTC 0.41–0.50 (BTC $h=7$: 0.413, $p=0.009$). Per-session votes (Buy/Sell only) hit 49–52%.


![Figure 3. Directional accuracy (95% Wilson) and 10–90 coverage by horizon.](output/figures/fig3_calibration_by_horizon.png)

By contrast, **interval calibration is good**: empirical 10–90 coverage is 0.78–0.82 against nominal 0.80 at every horizon, and the mean absolute standardized surprise is 0.77–0.89 (≈ the 0.80 expected for a well-calibrated Gaussian band). The model therefore knows *how wide* the distribution is in ordinary conditions but not *which side* the outcome falls on — a classic "calibrated but not sharp" forecaster.

### 4.2 Where a 60–65% reading comes from

The product's evaluator (`evaluate_forecast.py`) defaults to 50 origins sampled every 5 sessions. Re-sampling that design over the exhaustive results (2,000 random 50-origin windows) gives an $h=1$ accuracy of mean 0.514, SD 0.060 for BITO and mean 0.498, SD 0.065 for BTC; 14% (BITO) and 10% (BTC) of such windows print $\ge 0.60$, and 0.7–1.4% print $\ge 0.65$ (maximum observed 0.66 and 0.70). Recomputing what the evaluator would have displayed on each day of 2026 for BITO, the $h=5$ reading ranged 0.44–0.58 and never reached 0.60; the single best reading of any horizon was $h=4$ at 0.62 on 2026-06-26, and at least one of the seven horizons read $\ge0.60$ on 14% of days (Fig. 8). A remembered "61–63% on day 5" is thus most plausibly a noisy reading, an unrelated dataset, or a look-elsewhere effect across seven horizons; with $n=50$ the 95% Wilson interval around 0.62 is 0.48–0.74.
...
The 3-day published signal itself (2026, 133 active Long/Short days) has a next-day directional hit rate of 0.534 ($p=0.49$), ranging from 0.67 (Feb) to 0.385 (Jul); its +77% cumulative strategy return against $-13.6\%$ buy-and-hold comes from exposure asymmetry (inverse leg in the spring drawdown), not from directional accuracy.

### 4.3 Formal tests of predictive effectiveness in series

"Effectiveness in series" is made precise by three standard sequential criteria, each computed over the full origin sequence (Table 2). (i) The **Pesaran–Timmermann (1992) statistic** tests whether the predicted and realized directions are independent, correcting for unequal base rates; it is asymptotically $N(0,1)$, and values above 1.645 indicate skill at the 5% one-sided level. For $h>1$ consecutive outcomes overlap, so we also compute it on the $h$ non-overlapping subsequences (every $h$-th origin) and report their mean and range. (ii) A **Diebold–Mariano-type test** of the Brier loss against the constant forecast $\hat p\equiv\tfrac12$ (loss 0.25): $d_t=(\hat p_t-y_t)^2-0.25$, with Newey–West long-run variance (Bartlett kernel, lag $h$); a positive mean means the model is worse than the coin. (iii) The **cumulative log Bayes factor** $\Lambda_n=\sum_{t\le n}\big[y_t\log\hat p_t+(1-y_t)\log(1-\hat p_t)+\log 2\big]$ (probabilities clipped to $[0.02,0.98]$), whose per-step expectation under calibration equals the Kullback–Leibler divergence $\mathrm{KL}(\hat p_t\,\|\,\tfrac12)\ge0$ — so an effective probabilistic forecaster produces a submartingale drifting upward, and its even-odds Kelly bet of fraction $2\hat p_t-1$ has positive expected log growth. We report $\Lambda_n$, the share of the sequence spent below zero, and the realized Kelly growth per session.
...
Table 13 applies Benjamini–Hochberg control to every $p$-value reported in the paper (228 tests in ten pre-specified families; Section 3.6). The pattern is the paper's argument in one table. **No Pesaran–Timmermann skill test survives at any horizon on any instrument (0 of 56, within-family or pooled).** The *distributional* findings survive: the Brier score is worse than the constant forecast on 22 of 56 instrument–horizon pairs; the shock-window accuracy deficit survives on 5 of 8 instruments (BITO, SPXL, SPY, AAPL, MSFT); forecast surprise is larger on scheduled-event days in 10 of 16 instrument–calendar pairs (all four earnings sets, payrolls on SPY and SPXL, and BTC payrolls) and on headline-spike days for both crypto instruments. The 34 "direction significantly above ½" results that survive are all index or single-stock horizons whose always-up base rate exceeds the model's accuracy — base-rate inflation, not skill — and the 2 that survive *below* ½ are crypto horizons. The per-event *directional* results are the paper's weakest links once multiplicity is charged: the 37% pre-shock accuracy (BTC $q$=0.09 within family, 0.035 pooled; BITO $q$=0.26), the 39% headline-spike accuracy (BTC $q$=0.08) and the FOMC-day accuracy drop on SPY ($q$=0.60) are suggestive at $q\le0.10$ or weaker and should be read as such; the BTC seven-session continuation survives ($q$=0.023). The conclusions the paper rests on — no skill, confident miscalibration, and degradation of the predictive distribution on exogenous event days — are the ones that survive.
...
The coverage collapse is partly mechanical (Section 1). The **directional** result is not: for BITO, forecasts whose target span contains a shock are wrong 64% of the time, a 14-point deficit that survives every threshold from $|z|\ge2.0$ to $3.5$ (shock-window hit 0.36–0.40 vs calm 0.49–0.51). For BTC the deficit is smaller (0.46 vs 0.49) and not significant.
...
The 3-day signal is aligned with the coming shock exactly as often as a coin flip; the seven-session consensus almost never is (Fig. 7).
...
The deployed service applies one indicator-voter engine and one analog forecaster to crypto, the S&P 500 pair and several hundred single stocks; only the cash-mode trend/multi-horizon filters and the inverse leg differ. Running the identical protocol on the deployed S&P dataset (SPXL/SPXS, frozen 2026 signals), SPY spot and the four core stocks (AAPL, MSFT, TSLA, NVDA; frozen live signals from 2026-07-30) shows that the shared engine meets a different prediction problem in each type. (Fig. 9, 10)
...
To test whether four stocks are representative, the protocol was run on 46 further S&P 500 constituents from every sector with published `s00` datasets (live signals frozen from 2026-07-30; histories from 2010, 204,129 scored next-session forecasts; Berkshire Hathaway was dropped for lack of tagged headlines). Table 17 summarises the cross-section; the per-stock table is `output/universe_cross_section.csv`.
...
The cross-section sharpens the four-stock picture in one respect and confirms it in every other. Pooled over 204,129 forecasts the Pesaran–Timmermann statistic is 3.08: there *is* a statistically detectable directional dependence at one session in single stocks — of 0.32 percentage points above independence, and with 8% of stocks above the 5% critical value against 4% below. It is economically negligible and swamped by miscalibration: the Brier score is worse than a constant ½ for 98% of stocks (88% significantly), the always-up rule beats the model at seven sessions for 98% of stocks, and on earnings sessions the nominal-80% band covers 28% of outcomes for the median stock and less than its calm-session coverage for every one of the 48 stocks with earnings dates. Post-shock behaviour is mildly mean-reverting for the median stock (-0.18% over seven sessions; only 32% of stocks continue), the opposite of crypto. (Fig. 12.)
...
Conditional on not being aligned at $T-1$, the 3-day signal first agrees with the shock direction after a median of 2 (BITO) and 3 (BTC) sessions; 24 of 98 BTC events never produced agreement within 15 sessions. The seven-vote consensus needs a median of 5 (BITO) and 6 (BTC) sessions (Fig. 7). Given the drift in Table 7, **the model's agreement arrives after most of the exploitable continuation has passed**, and on the BTC instrument the 3-day signal's cash-mode trend filter forces "Long" for long stretches regardless, so "alignment" there is partly coincidental.
...
Two lessons. First, **directional accuracy is the wrong yardstick**: by the $\hat p\ge0.5$ rule this forecast "hit" six of seven horizons, while the realized path lay 4–8 band-$\sigma$ above every median and outside every interval. Second, the payload's two exposure mechanisms disagreed: the analog-schedule target said $-100\%$ (its score was dominated by the negative 3-day signal), while the vote consensus said Hold. The published strategy return on Aug 19 was $-5.86\%$ (long BITI). The 3-day signal flipped to Long at the Aug 19 close (after the first +6%), the consensus reached "Buy / warning" only on Aug 21 (after +19%), and the signal flipped back to Short with a $-100\%$ target on Aug 24. For BTC spot the forecast issued Aug 18 was worse: consensus **Sell / confirmed (0 Buy, 6 Sell)**, $h=1$ band $[-1.6\%,+1.2\%]$ versus +7.15% ($s=6.7$), $h=3$ $s=9.5$, 0/7 coverage, 1/7 direction hits. A "news chaser" who bought BITO at the Aug 19 close held +6.1%, +12.5%, +14.9% and +14.5% after 1, 2, 3 and 5 sessions.
...
If the failure is the absence of an event state variable, then adding one — even crudely — should improve the predictive distribution exactly where the paper says it fails, and nowhere else. We test this out of sample with **causal gates** built only from information available at the origin and thresholds fixed in Section 3: *calendar* (the target session is an FOMC/payroll day for the index or an earnings reaction session for a stock), *headline* (the previous session's own-news ratio $\nu_{t-1}\ge2$), *shock* (the origin session itself was a $\lvert z\rvert\ge2.5$ move). On gated forecasts the model's quantiles are replaced by an **event-conditional distribution estimated walk-forward from the past only**: the empirical quantiles of past same-type event sessions (earnings, FOMC/payroll; ≥8 events), or the sign-adjusted post-shock continuation distribution (≥10 shocks); otherwise a trailing 250-window climatology. Scores are the Winkler interval score (α=0.2), coverage and Brier; differences are block-bootstrapped. (Table 18, Fig. 13.)
...
The analog forecaster is contrarian after large moves because its neighbours come from calm regimes in which large one-day moves partially revert; 21 of 98 BTC shocks and 10 of 48 BITO shocks were followed by another shock within three sessions, almost always of the same sign (Aug 19–21 2026 being the cleanest case). Proposition 2 therefore predicts the observed anti-skill on shock sessions (Table 3: PT $-2.49$ for BTC; $h=1$ accuracy 0.37).
...
which equals $0.111$ at $c=2.5$ and $0.013$ at $c=3.5$. Hence observed coverage of $0.000$ on $|z|\ge2.5$ days lies inside the mechanical bound and carries no information about the forecaster; the informative coverage statistics are those conditioned on headline intensity (Table 5), which does not fix $c$.
...
**Limitations.** (1) Offline reproduction differs from production in the rotation feature and price source (76% unfrozen signal agreement); the 2026 signals are exact. (2) News intensity is a Benzinga headline count, a crude proxy for information content, salience and sentiment. (3) Two crypto instruments, two index instruments and 50 stocks; the crypto window is 4.5–5.5 years with 48 and 98 shock events; the 46 universe stocks were not fidelity-checked against production individually. (4) Overlapping horizons induce dependence; we use block bootstraps but the effective sample is smaller than the row count. (5) Seven horizons × multiple splits invite multiplicity; we report all horizons and the reader should treat $p\approx0.03$ results as suggestive. (6) Survivorship is not an issue (single instruments), but regime dependence is: 2024–2026 includes an ETF-approval bull run, a 2026 drawdown and a policy-driven rally.

### 7.3 The predictability gap

Table 16 puts the ceiling-and-floor argument of Proposition 4 to the data. On every instrument, sessions with $|z|\ge2.5$ are only 3.4–5.0% of the sample yet carry **28.5–36.5% of the return variance** and 13–18% of absolute price movement; headline-spike days add a further 5–14% of sessions. These are the sessions on which a price-conditioned model's directional accuracy is, by Proposition 1, pinned near the news-sign base rate (observed 0.37–0.56) and its interval coverage is mechanically near zero. Calm-session accuracy — the only place skill could live — is 0.49–0.53, indistinguishable from the coin and *below* the always-up base rate on the drifting instruments. The arithmetic follows: had the model possessed 60% (70%) directional skill on calm sessions, its overall accuracy could still not have exceeded ≈0.60 (≈0.69). The 60–70% figures advertised by retail signal products are therefore not a plausible *average*; they are the *ceiling under substantial calm-period skill that this model — and, on the evidence of Section 1.1, most technical models — does not have*. Between the ceiling and the coin lies the gap this paper measures: roughly a third of all price variance arrives on days the model treats as ordinary, and nothing conditioned on past prices can move that third.
...
Two limitations of the gap measure. It uses the return-defined shock set, so the "unpredictable" share is a lower bound — adding headline-spike and calendar sessions (Sections 5.2, 5.5) raises it; and a model with a genuinely different information set (news text, order flow, options positioning) is not bound by Proposition 1 — the bound is on *price-conditioned* predictors, which is what the deployed engine and the technical-rule literature are.
...
The central empirical fact of this paper is that the errors of a pattern-based forecaster are not white: they are concentrated on and after news arrivals and have a *sign* (the model fades moves that continue). That structure is produced by people. A model that wants to survive news must therefore contain a model of how people respond to news — which is a neuroscience question with a mathematical interface.
...
Evaluated exhaustively and point-in-time, a deployed seven-session analog forecaster for Bitcoin instruments is directionally uninformative (46–51% at every horizon), well-calibrated in width during calm periods, and systematically wrong in sign and width when exogenous news arrives: 37–39% next-session accuracy on shock and headline-spike days, zero interval coverage the day before a shock, and a mean surprise of 3.6 band-$\sigma$. Prices continue in the shock direction for about a week while the model's consensus takes 5–6 sessions to agree. The 2026-08-19 CLARITY-Act rally, on which the model recommended a fully inverse position, is a textbook instance. The failure is structural — human response to information is not a function of past prices — and points to a joint mathematical and neuroscientific program to measure and model that response. It is also type-specific: the same engine is contrarian into continuing shocks on crypto, base-rate-inflated and collapse-prone on the negatively skewed index, and degenerate on single stocks whose shocks are idiosyncratic own-news jumps; and no way of acting on its schedule (daily, weekly, once) dominates across instruments. Coin, index and single stocks need separate model families evaluated against their own base rates — and the constructive test shows what such a family buys: an event-conditional gate estimated from the past alone restores coverage on earnings sessions (0.49→0.68 across 50 stocks, interval score +15%) and after crypto shocks, while the wrong gate for a type makes things worse. The gap is quantified: 3–5% of sessions carry 28–37% of return variance and are unpredictable in sign from prices, so even 60–70% calm-period skill would cap accuracy near 0.60–0.69, and the observed calm-period skill is nil. For a user the operational content is narrow: after a crypto shock act within one session or not at all; after index and stock moves, do not chase. Closing any part of that gap requires modelling the human reaction to news and social media — the attention, arousal and herding chain of Section 8.4 — rather than the price path that follows it.
